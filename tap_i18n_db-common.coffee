TAPi18n.Collection = (name, options = {}) ->
  ...
  collection = share.i18nCollectionExtensions(
    commonCollectionExtensions new Meteor.Collection name, options
  )
  ...
  collection._base_language = if options.base_language
    options['base_language']
  else
    globals.fallback_language
...
commonCollectionExtensions = (obj) ->
  ...
  verifyI18nEnabled = (attempted_operation, callback) ->
    ...
    throwError new Meteor.Error(400, 'TAPi18n is not supported'),
      attempted_operation, callback

  isSupportedLanguage = (lang, attempted_operation, callback) ->
    if lang in TAPi18n.conf.supported_languages
      return

    throwError new Meteor.Error(400, "Not supported language: #{lang}"),
      attempted_operation, callback
  ...
  getLanguageOrEnvLanguage = (language_tag, attempted_operation, callback) ->
    ...
    throwError new Meteor.Error(400, "Missing language_tag"),
      attempted_operation, callback
  ...
  #       else
  #         _.extend updates, _.object _.map translations[lang],
  #           (val, field) -> ["i18n.#{lang}.#{field}", val]
  #
  #   @.update.apply @,
  #     removeTrailingUndefs [selector, {$set: updates}, options, callback]
  #
  ...
  #
  #   if not fields?
  #     reportError new Meteor.Error(400, "Missing arugment: fields"),
  #       "remove translations", callback
  #     return null
  #
  #   if not _.isArray fields
  #     reportError new Meteor.Error(400, "fields argument should be an array"),
  #       "remove translations", callback
  #     return null
  #
  #   updates = {}
  ...
  #         reportError new Meteor.Error(400, "Complete removal of collection's base language from a document is not permitted"),
  #           "remove translations", callback
  #         return null
  #
  #       updates[field] = ""
  ...
  #
  #   @.update.apply @,
  #     removeTrailingUndefs [selector, {$unset: updates}, options, callback]
